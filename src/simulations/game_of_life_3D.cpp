...
double GameOfLife3D::Step(double dt) {
    auto [rows, cols, stacks] = this->gridSize.elements;
    std::vector<uint8_t> next_cells = this->cells; // TODO generated by copilot - inefficient, mallocs every Step

    // TODO order
uint32_t stack = 0;//    for (int32_t stack = 0; stack < static_cast<int32_t>(stacks); stack++) {
